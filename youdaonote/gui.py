...
class YoudaoNoteGUI:
    ...
    def create_status_area(self, parent):
        ...
        # 下载目录显示 - 从配置读取默认目录
        default_download_dir = self._get_default_download_dir()
        self.download_dir_var = tk.StringVar(value=f"下载目录: {default_download_dir}")
    ...
    def login(self):
        ...
        try:
            ...
            # 初始化工具类（使用新的类名）
            self.searcher = YoudaoNoteSearch(self.youdaonote_api)
            self.downloader = YoudaoNoteDownload(self.youdaonote_api)
    ...
    def load_directory_contents(self, dir_id: str):
        ...
        def load_in_thread():
            try:
                ...
                self._run_on_ui(_update_ui)

            except Exception as e:
                error_msg = f"加载目录内容失败: {e}"
                self.safe_set_status(error_msg)
                self._run_on_ui(lambda: messagebox.showerror("错误", error_msg))
            ...
            for entry in dir_info.get('entries', []):
                ...
                if is_dir:
                    self._search_recursively(entry_id, target_name, search_type, exact_match,
                                           results, current_entry_path, depth + 1)
    ...
    def _download_item(self, item):
        ...
        try:
            ...
            self._run_on_ui(lambda: messagebox.showinfo("成功", f"'{safe_name}' 下载完成!"))
    ...
    def _download_single_item(self, item):
        ...
        if is_dir:
            self.downloader.download_folder(item_id, safe_name, self.download_dir)
        else:
            self.downloader.download_file(item_id, safe_name, self.download_dir)
